...
class Wallet:
    ...
    Allows a miner to auth transactions.
    """
    ...
    def __init__(self, blockchain=None):
        ...
        self.private_key = ec.generate_private_key(
            ec.SECP256K1(), 
            default_backend()
        ) #standard of efficient cryptography, prime
    ...
    def sign(self, data):
        """
        Generate a signature based on the data using local pvt key
        """
    ...
    def calculate_balance(blockchain, address):
        """
        Calculate the balance of given address considering the transaction data within blockchain
...
def main():
    ...
    x = Wallet.verify(wallet.public_key, data, signature)
    print(f'should_be_valid: {x}')

    y = Wallet.verify(Wallet().public_key, data, signature)
    print(f'should_be_invalid: {y}')
